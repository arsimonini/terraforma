...
[SerializeField]
public class BubbleBomb_Spell : MonoBehaviour, Cast_Spell
{
    public int spread;
    ...
    public void castSpell(List<GameObject> targets, GameObject caster)
    {
        for (int i = 0; i < targets.Count; i++)
        {
            ...
            GameObject newTile = (GameObject)Instantiate(tileWaterGrass, new Vector3(previousTile.gameObject.transform.position.x, 0, previousTile.gameObject.transform.position.z), Quaternion.identity);
            ...
            */
            ...
            newEffect.createTileEffect(true, targetTile, newSource: "BubbleBomb", newDuration: 3);
            */
            /* Wall of Water
            ClickableTile targetTile = targets[i].GetComponent<ClickableTile>();
            RaycastHit[] hits;
            ...
            */
                    ...
                    if (hitCollider.gameObject.name.StartsWith("tile")){
                        if (hitCollider.gameObject.tag != "Wall"){
                            if (hitCollider.gameObject.GetComponent<ClickableTile>().characterOnTile != null){
                                hitCollider.gameObject.GetComponent<ClickableTile>().characterOnTile.GetComponent<Basic_Character_Class>().takeMagicDamage(damageScale*caster.GetComponent<Hero_Character_Class>().magic.moddedValue, "Water");
                            }
                            hitCollider.gameObject.GetComponent<ClickableTile>().map.gameObject.GetComponent<ReactionController>().checkReaction(hitCollider.gameObject.GetComponent<ClickableTile>(), caster.GetComponent<Hero_Character_Class>().selectedSpell.elementType, "BubbleBomb", true);
            ...
            if (targets[i].GetComponent<Basic_Character_Class>()){
                ...
                targets[i].GetComponent<Basic_Character_Class>().takeMagicDamage(caster.GetComponent<Hero_Character_Class>().magic.moddedValue, "Water");
            }
            else {
            ...
            //TileEffect newEffect = Instantiate(BurningTileEffect);
            //newEffect.createTileEffect(true, targetTile, newSource: "BubbleBomb", newDuration: 3);
            targetTile.map.gameObject.GetComponent<ReactionController>().checkReaction(targetTile, caster.GetComponent<Hero_Character_Class>().selectedSpell.elementType, "BubbleBomb", true);

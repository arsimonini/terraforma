...
public class Pirates_Eye : MonoBehaviour, Cast_Spell
{
    [SerializeField]
    public BuffClass PiratesEyeBuff;
    //public int startHp;
    ...
    public void castSpell(List<GameObject> targets, GameObject caster){
        ...
        if ((targets.Count) > 0 && (targets[0].GetComponent<Basic_Character_Class>() != null)) {
            ...
            for (int i = 0; i < ally.GetComponent<Basic_Character_Class>().buffs.Count; i++){
                if (ally.buffs[i].name == "Pirate's Eye"){
                    ally.buffs[i].duration += 3;
                    return;
                }
            }
        
            BuffClass newBuff = Instantiate(PiratesEyeBuff);
